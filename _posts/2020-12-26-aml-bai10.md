...
1.   Gaussian NB: Sử dụng hàm mật độ xác suất (probability density function) của phân phối chuẩn để tính khả năng (likelihood). Thích hợp với các tập dữ liệu (dataset) có dữ liệu dạng (numerical value).
...
Tập dữ liệu (dataset) của chúng ta bao gồm cả dữ liệu dạng số (numerical value) cả dữ liệu dạng phân loại (categorical value), và cả dữ liệu phân phối dưới dạng nhị phân (Các cột được thực hiện One-hot encoding). Do đó chúng ta sẽ thử lần lượt từng loại NB model và xem model nào cho kết quả tốt nhất nhé.
...
```
    ...
    <matplotlib.legend.Legend at 0x7fce6095d978>
...
Có thể thấy Multinomial NB là model có độ chính xác (accuracy) trên cả tập huấn luyện (training set) và tập xác nhận (validation set) tốt hơn cả so với 2 model còn lại. Có thể lý giải nguyên nhân Gaussian NB có độ chính xác thấp nhất vì phần lớn các biến không phụ thuộc (dataset) này có dữ liệu dạng phân loại (categorical value). Việc sử dụng mật độ xác suất (probability density) theo phân phối chuẩn (normal distribution) trên các dữ liệu dạng này gây sai sót cho model.

# **Kiểm thử Naive Bayes model đã huấn luyện**

Chúng ta sẽ kiểm thử Naive Bayes model tốt nhất đó là Multinomial NB trên tập dữ liệu kiểm thử (test data) và so sánh với Logistic Regression model chúng ta đã huấn luyện (training) ở bài 8 nhé.
...
Chúng ta sẽ lần lượt kiểm tra các chỉ số độ chính xác (accuracy), độ bao phủ (recall / sensitivity), độ chuẩn xác (precision), ma trận nhầm lẫn dự đoán của Multinomial NB model trên tập huấn luyện (testing set).
