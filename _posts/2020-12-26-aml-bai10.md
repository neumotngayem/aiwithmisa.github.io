...
Chúng ta sẽ lần lượt kiểm tra các chỉ số độ chính xác (accuracy), độ bao phủ (recall), độ chuẩn xác (precision), ma trận nhầm lẫn (confusion matrix) để kiểm tra hiệu năng (performance) của Multinomial NB model trên tập huấn luyện (testing set).
